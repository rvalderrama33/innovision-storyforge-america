-- Add random 4-5 star reviews using existing users
DO $$
DECLARE
    ...
    random_user_id UUID;
BEGIN
    -- Get existing user IDs and names
    FOR user_record IN 
        SELECT id, full_name FROM profiles WHERE full_name IS NOT NULL
    LOOP
        existing_users := array_append(existing_users, user_record.id);
        user_names := array_append(user_names, user_record.full_name);
    END LOOP;
    
    -- Only proceed if we have users
    IF array_length(existing_users, 1) > 0 THEN
        -- Loop through each active marketplace product to add reviews
        ...
        LOOP
            ...
            FOR i IN 1..review_count LOOP
                ...
                -- Pick random user from existing users
                random_user_id := existing_users[1 + floor(random() * array_length(existing_users, 1))];
