...
serve(async (req) => {
  ...
  try {
    ...
    // Send emails in batches to avoid rate limiting (same approach as newsletter)
    const batchSize = 3; // Same batch size as newsletter function
    ...
    for (let i = 0; i < submissions.length; i += batchSize) {
      ...
      // Add delay between batches to respect rate limits (same as newsletter)
